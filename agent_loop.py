...
def _synthesize_tool_call(tool_name: str, args: dict, tool_executor, messages: list, emitter, stats, self_check_fn=None) -> str:
    ...
    while True:
        ...
        if not _force_finish and tools and raw_reply:
            ...
            if extracted:
                ...
                # instead. Route through the same pre-dispatch safety gate
                # as native tool calls so a ``<tool_call>`` regex hit can't
                # bypass shell-safety / write-whitelisting.
