...
EDITABLE_SETTINGS = {
    ...
    "telemetry_endpoint":     ("setting:telemetry_endpoint",    str, "",    "URL to POST batched telemetry events to. Empty = no network calls (events queue locally for inspection). Self-hosted users can point this at their own Plausible / PostHog / custom collector. For Plausible: full URL including /api/event path (e.g. https://plausible.example.com/api/event).", "", ""),
    "telemetry_format":       ("setting:telemetry_format",      str, "raw", "Wire format. 'raw' = single POST with our JSON shape ({events: [...]}) suited for custom collectors. 'plausible' = per-event POST in Plausible's /api/event format with synthetic IP derived from anonymous_id for stable unique-visitor counting.", "", ""),
    "telemetry_plausible_domain": ("setting:telemetry_plausible_domain", str, "", "Site domain registered in your Plausible dashboard (e.g. 'qwe-qwe.app'). Required when telemetry_format=plausible — Plausible rejects events whose domain doesn't match a registered site.", "", ""),
