...
1. **Per-repo (авторитативный для refresh)** — отдельная точка `kind=index_state`, `id = UUIDv5("index_state", <repo>)`, payload `{repo, last_indexed_commit, last_indexed_ts}`. Обновляется атомарно в конце каждого успешного `qctx index` / `qctx refresh`.
...
```
1. repo_state   = Qdrant.get(id=UUIDv5("index_state", <repo>))
2. old_commit   = repo_state.last_indexed_commit                # единое для репо
3. new_commit   = git rev-parse HEAD
4. if old_commit == new_commit and working tree clean:
       → "nothing to do"; return
5. diff = git diff --name-status <old_commit>..<new_commit>      # A/M/D/R
6. if working tree dirty:
       diff += git status --porcelain                             # uncommitted
7. обрабатываем per-file правила (см. ниже)
8. Qdrant.upsert(id=UUIDv5("index_state", <repo>),
                 payload={last_indexed_commit: new_commit, last_indexed_ts: now()})
```

`kind=index_state` — ещё один kind в payload-indexes (добавлен в §5). Это mini-metadata-коллекция, не создаёт шума в search.
...
- **Renamed (git `R` статус):** ID меняется (UUIDv5 завязан на `file_path`), технически это delete+add двух точек. Чтобы не терять LLM-работу:
  1. Создаём новую точку с `new_id = UUIDv5(kind, repo, new_path)`
  2. Копируем из старой payload семантические поля: `purpose`, `public_api`, `depends_on`, `tags`, `parent_module`, `parent_project`, `signature_hash`
  3. Обновляем path-зависимые: `file_path`, `updated_ts`, `source_commit`
  4. Во всех entity `mentioned_by`: `<old_id>` → `<new_id>`
  5. Удаляем старую точку
