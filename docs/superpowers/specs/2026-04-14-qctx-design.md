...
| `index_state` | per-repo metadata (last_indexed_commit, ts); не эмбеддится, не показывается в search | — |
...
- `code_skeleton`: `file_path`, `language`, `imports: [string]`, `exports: [string]`, `public_symbols: [string]`, `first_doc_comment_line: string?`, `module_docstring: string?`, `loc`
- `module_card`: `module_path`, `parent_project`, `contains_files: [id]`
- `project_card`: `name`, `stack: [string]`, `purpose`, `entry_points: [string]`, `related_repos: [id]` (reserved для v2 — в v1 payload присутствует как пустой массив, не заполняется; будет использован для cross-project similarity graph)
- `doc_chunk`: `file_path`, `chunk_index`, `chunk_total`
- `entity`: `name`, `entity_type`, `relations: [{to, rel, weight}]`, `mentioned_by: [id]`, `aliases: [string]`, `tombstoned_at?: float`
...
**Namespace UUID:** `NS_QCTX = UUIDv5(NAMESPACE_DNS, "qctx")` — фиксированный константой в коде (не строка). Конкретное значение: `8f14e45f-ceea-567e-a27d-db6baf3f7e34`. Все бинари (CLI и MCP) используют одну константу, иначе IDs расходятся.

**ID = UUIDv5(NS_QCTX, `"{kind}:{repo}:{path}:{extra}"`)** — детерминированный, повторная индексация = upsert в ту же точку.

**Поля `{path}` и `{extra}` по kind:**
| kind | {path} | {extra} |
|---|---|---|
| `file_card`, `code_skeleton` | `file_path` | `""` |
| `module_card` | `module_path` | `""` |
| `project_card` | `""` | `""` |
| `doc_chunk` | `file_path` | `chunk_index` (строка) |
| `entity` | `""` | `normalized_name` |
| `note` | `scope_path \|\| ""` | `sha256(text)[0..16]` — первые 16 hex-символов; одинаковый текст = upsert, разный = новая точка |
| `index_state` | `""` | `""` → итог: `UUIDv5(NS_QCTX, "index_state:{repo}::")` |
| `wiki` | `""` | `slug` |
...
- keyword: `kind`, `repo`, `language`, `entity_type`, `parent_project`, `parent_module`, `scope_kind`, `scope_repo`, `scope_path`, `scope_entity`, `author`, `name`, `file_path`
...
> `name` и `file_path` — критичны для hot-path: entity-dedup в Фазе 5 (десятки тысяч lookups на bulk-run) и deleted/renamed lookups в refresh. Без индексов — full-collection scan, O(N) на каждый вызов.
> `scope_path` и `scope_entity` нужны для фасет-фильтрации заметок `remember()`.

**`index_state` points** живут в той же коллекции `qctx_knowledge`, но без векторов. Qdrant ≥1.10 поддерживает payload-only точки в named-vector коллекции — `index_state` апсертируется без полей `vectors`. Минимальная версия Qdrant для qctx: **1.10**.
...
Извлекаем: `imports`, `exports`, `public_symbols` (= top-level публичные функции/классы/константы), `first_doc_comment_line` (первая строка leading doc-comment или module docstring), `module_docstring` (полная module-level docstring, если есть), `language`, `loc`.

Для Python: `module_docstring` — `(module . (expression_statement (string)))` node. Для JS/TS: leading `/** ... */` JSDoc block. Эти поля входят в `signature_hash` (§8), поэтому обязательны.
...
**Приоритет privacy (от строгого к мягкому):** `.qctx.toml privacy = "local"` — **жёсткий пол, не переопределяется никем**. Если MCP-инструмент вызывает `index_repo(path, privacy="cloud")` для репо с `.qctx.toml: privacy = "local"` — инструмент возвращает ошибку `{state: "rejected", reason: "repo .qctx.toml pins privacy=local"}`, код репо в удалённый LLM не отправляется. Во всех остальных случаях: MCP arg > `.qctx.toml` > global config > default. Смысл: пользователь отметил репо как приватное — ни агент, ни случайный вызов не должен это перебить молча.
...
2. Payload-фильтр `kind=entity AND name=<normalized>` — поиск существующей (индекс по `name` обязателен, см. §5)
3. Existing → `append file_id → entity.mentioned_by`
4. New → create с `type="tech"` (эвристика: суффиксы `-db`, whitelist, иначе "concept")

**Конкурентность entity-апдейтов:** шаги 2–4 — non-atomic read-modify-write. При N=8 воркерах несколько воркеров могут одновременно обрабатывать файлы с одинаковым `normalized_name`. Qdrant не поддерживает CAS на payload-массивах — last-write-wins затрёт `mentioned_by` предыдущего воркера.

Решение — **per-entity-name in-process mutex** (sharded by hash of name):
- Перед шагами 2–4 воркер захватывает `entity_locks[hash(normalized_name) % 256]` (256 шардов достаточно при N≤8)
- Держит lock на время шагов 2–4, освобождает после upsert
- Неперекрывающиеся entity обрабатываются параллельно, hot-entities сериализуются

Это не влияет на межпроцессный lock (§8 Concurrency) — тот защищает от параллельных CLI-запусков. Внутрипроцессный шардированный mutex защищает от внутри-процессных воркеров.

**Alias-таблица** — 3-уровневая. Правило приоритета: **более специфичный источник побеждает**. Резолв реализуется как merge с shadowing (не first-match-wins по порядку номеров):

1. **Runtime в Qdrant** (наивысший приоритет) — при `qctx entity link A --alias-of B` или через CLI. Хранится как payload-поле `aliases: [...]` у самого entity-ноды. Поиск при резолве идёт в две стороны: по `name` и по `aliases`.
2. **User config** — `~/.config/qctx/aliases.toml`, override/extension builtin. Редактируется вручную. Перекрывает Builtin-записи с тем же ключом.
3. **Builtin** (seed, зашит в бинарник, наименьший приоритет) — `qctx-core/src/aliases.toml`, ~300 пар: популярные технологии, их ребрендинги, общепринятые сокращения. Обновляется с релизами. Примеры: `postgres → postgresql`, `nodejs → node`, `ts → typescript`, `k8s → kubernetes`.

Итоговая таблица строится: сначала Builtin, затем User config мержится поверх (перекрывает совпадающие ключи), затем Runtime Qdrant мержится поверх User. Lookup всегда идёт по итоговой merged-таблице.
...
**Discovery:**
- `list_projects(stack?, has_tag?)` → scroll по `kind=project_card`, дополнительно включает `kind=note AND scope_kind=project` (заметки, привязанные к проектам через `remember("project:<repo>", ...)`)
...
- `remember(scope, text, tags?)` → создаёт note-точку, author=claude-code, эмбеддинг on-the-fly
  ...
  **Парсинг scope** в поля `scope_kind`, `scope_repo`, `scope_path`, `scope_entity`:
  - `scope_kind` = подстрока до первого `:`
  - Для 2-частных форм (`project:<repo>`, `entity:<name>`): `split(":", maxsplit=1)` → второй элемент = repo или entity_name. Лишние `:` в значении недопустимы и дают ошибку.
  - Для 3-частных форм (`module:<repo>:<path>`, `file:<repo>:<path>`): `split(":", maxsplit=2)` → repo + path. Путь может содержать `:` (редко, но возможно в macOS/POSIX) — они сохраняются без эскейпинга.
  - `global` — без разделителей, лишние символы дают ошибку.
  - Невалидный scope → tool возвращает ошибку, note не создаётся.

  **Все read-инструменты** (`search`, `get_card`, `find_similar`, `related`, `list_stacks`) неявно добавляют фильтр `NOT tombstoned_at` (т.е. `tombstoned_at IS NULL OR tombstoned_at > now()`). Мёртвые entity не видны до purge. Исключение: `qctx doctor` — показывает tombstone-сущности явно для диагностики.
...
### Источник правды для "что изменилось"

**Два уровня `source_commit`:**

1. **Per-repo (авторитативный для refresh)** — отдельная точка `kind=index_state`, `id = UUIDv5(NS_QCTX, "index_state:{repo}::")`, payload `{repo, last_indexed_commit, last_indexed_ts}`. Точка без векторов (payload-only, Qdrant ≥1.10). Обновляется атомарно в конце каждого успешного `qctx index` / `qctx refresh`.
2. **Per-point (для observability)** — поле `source_commit` в каждой content-точке. Используется `qctx doctor` чтобы находить точки, отставшие от `index_state` (признак багнутой индексации). На refresh-path **НЕ читается** — нет риска "pick min/max по файлам".

**Алгоритм `refresh <repo>`:**

```
1. repo_state = Qdrant.get(id=UUIDv5(NS_QCTX, "index_state:{repo}::"))
   if not found:
       → error {state: "not_indexed", reason: "run `qctx index <path>` first"}; return
2. old_commit  = repo_state.last_indexed_commit
3. new_commit  = git rev-parse HEAD
4. tree_dirty  = (git status --porcelain) не пустой
4b. if old_commit == new_commit and not tree_dirty:
       → "nothing to do"; return
5. Нормализуем diff в единый список (status, old_path, new_path?):
   a. git diff --name-status <old_commit>..<new_commit>
      Парсим построчно: "A\tpath", "M\tpath", "D\tpath", "R100\told\tnew"
      → [(Added, path), (Modified, path), (Deleted, path), (Renamed, old, new)]
   b. if tree_dirty:
      git status --porcelain=v1
      Парсим XY-коды: "?? path" → (Added, path), " M path" → (Modified, path),
      "R  old -> new" → (Renamed, old, new), итд.
      Результат merge в тот же список (status, old_path, new_path?)
6. Обрабатываем per-file правила (см. ниже)
7. if not tree_dirty:
       Qdrant.upsert(id=UUIDv5(NS_QCTX, "index_state:{repo}::"),
                     payload={last_indexed_commit: new_commit, last_indexed_ts: now()})
   else:
       # Не обновляем last_indexed_commit — иначе следующий refresh
       # пропустит uncommitted изменения если их откатят (git checkout --)
       Qdrant.upsert(..., payload={last_indexed_ts: now()})
       # dirty_files записываем отдельно для observability
```

> Почему `--porcelain=v1` для uncommitted: единый стабильный формат без зависимости от locale и git-версии. Оба источника нормализуются в `(StatusCode, old_path, new_path?)` перед merge — один парсер для обоих.

`kind=index_state` — ещё один kind в payload-indexes (добавлен в §5). Не создаёт шума в search — все read-инструменты фильтруют по нужным kind-значениям, `index_state` в них не входит.

**Per-file handling:**

- **Modified:** re-skeleton (включая `public_symbols`, `first_doc_comment_line`, `module_docstring` — все поля из §5 code_skeleton), upsert. Считаем `signature_hash = sha256(canonical_json({imports, exports, public_symbols, first_doc_comment_line, module_docstring}))` — поля, которые семантически влияют на то, что LLM скажет про файл. LOC, whitespace, комментарии внутри тел функций НЕ входят — чтобы косметические правки не триггерили re-LLM. Если `signature_hash` изменился ИЛИ `--force-llm` → re-LLM. Иначе только skeleton обновляется. Diff `depends_on` old vs new → update entity.mentioned_by на обеих сторонах.
...
- **Deleted:** удалить все точки с `file_path=X AND repo=R`, убрать `file_id` из всех entity.mentioned_by, обновить module_card.contains_files.
  ...
  **Orphan entities** (mentioned_by=[] после удаления последней ссылки):
  ...
  3. Очистка tombstone'ов — ленивая: выполняется в начале каждого `qctx refresh`, `qctx index` и `qctx doctor`. Запрос: `filter(kind=entity AND tombstoned_at < now()-86400 AND mentioned_by is_empty)` → delete.
     > **Важно:** условие `mentioned_by is_empty` — safety net против race между revival и purge. Если воркер успел добавить `mentioned_by`, но ещё не стёр `tombstoned_at` (crash/partial write), purge не удалит живую entity. Отдельный cron/демон не нужен.
  4. `qctx doctor` явно показывает tombstone count: `tombstoned entities (will be purged on next op): 3`
- **Renamed (git `R` статус):** ID меняется (UUIDv5 завязан на `file_path`), технически это delete+add двух точек. Чтобы не терять LLM-работу:
  1. Создаём новую точку с `new_id = UUIDv5(NS_QCTX, "file_card:{repo}:{new_path}:")`
  2. Копируем из старой payload **только LLM-семантические** поля: `purpose`, `public_api`, `depends_on`, `tags`, `signature_hash`
     > `parent_module` и `parent_project` — path-derived, **не копировать**: они зависят от `new_path` и должны быть пересчитаны на шаге 3.
  3. Вычисляем и записываем path-зависимые поля из `new_path`: `file_path`, `parent_module`, `parent_project`, `updated_ts`, `source_commit`
  4. Во всех entity `mentioned_by`: заменяем `<old_id>` → `<new_id>`
  5. Обновляем `module_card.contains_files`:
     - Если `parent_module` изменился: удаляем `old_id` из старого `module_card.contains_files`, добавляем `new_id` в новый `module_card.contains_files`
     - Если `parent_module` не изменился: swap `old_id` → `new_id` in place
  6. Удаляем старую точку

  LLM повторно НЕ вызывается, если `signature_hash` не изменился. Если рефакторинг одновременно переименовал И поменял публичное API — после переноса сработает обычный modified-path через diff signature_hash.
