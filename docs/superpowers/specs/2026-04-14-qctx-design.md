...
- `fastembed` — ONNX embeddings локально (MiniLM-L12-v2 384d + SPLADE++)
...
- keyword: `kind`, `repo`, `language`, `entity_type`, `parent_project`, `parent_module`
- keyword array: `stack`, `tags`, `depends_on`, `mentioned_by`
- float: `updated_ts`
- text (full-text): `text` — word tokenizer, lowercase

**Vectors config:**
- `dense`: 384d float16, cosine (MiniLM-L12-v2)
- `sparse`: SPLADE++ с `modifier=idf`
...
1. Нормализация имен в `depends_on` (casefold + alias-таблица: `postgres == PostgreSQL == psql`)
...
Автоматическая дедупликация entities cross-repo. Ручной retype через `qctx entity retype`.
...
- **Modified:** re-skeleton, upsert, если хэш сигнатуры изменился ИЛИ `--force-llm` → re-LLM. Diff `depends_on` old vs new → update entity.mentioned_by на обеих сторонах.
- **Added:** полный пайплайн.
- **Deleted:** удалить все точки с `file_path=X AND repo=R`, убрать `file_id` из всех entity.mentioned_by, обновить module_card.contains_files. Orphan entities (mentioned_by=[]) — tombstone на 24ч, потом удалить.
...
**Concurrency:** CLI держит `~/.cache/qctx/lock` (fcntl). MCP `index_repo` и `refresh` берут тот же lock, падают с понятным сообщением при конфликте. Background jobs в MCP — через tokio JoinSet, N одновременно.
